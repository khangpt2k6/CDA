...
\begin{itemize}
  \item SN74LS04N inverter IC
  \item MM74HC11N 3-input AND gate IC
  \item SN74LS86AN 2-input XOR gate IC
  \item SN74LS153N dual 4-to-1 multiplexer IC
  \item Breadboard
  \item Jumper wires
...
We started by writing out the truth tables for the 2-to-1 and 4-to-1 multiplexers and then derived the SOP expressions from those tables. Each product term in the 4-to-1 mux SOP has three inputs (two select lines and one data input). We used the MM74HC11N which provides 3-input AND gates directly, so each product term maps to exactly one gate without any cascading needed. No OR gate was required in this implementation.
...
\begin{itemize}
  \item 2 inverters (from SN74LS04N) to generate $\overline{S_0}$ and $\overline{S_1}$
  \item 2 three-input AND gates (from MM74HC11N) for the product terms
\end{itemize}

For the comparator circuit, we used 1 XOR gate (from SN74LS86AN) and 1 additional inverter (from the same SN74LS04N). No OR gate was used in this implementation.

In total, the entire setup used: 1 x SN74LS04N (inverters), 1 x MM74HC11N (AND gates), 1 x SN74LS86AN (XOR comparator), plus the SN74LS153N as the reference mux.
...
One thing we noticed during the build is that the custom mux still takes up noticeably more space on the breadboard compared to just plugging in the SN74LS153N. Even with the MM74HC11N providing 3-input AND gates directly and eliminating the need for an OR gate, the wiring between inverters, AND gates, and the comparator adds up quickly. This is a good example of why higher level ICs exist: they save board space, reduce wiring complexity, and are less prone to mistakes.
