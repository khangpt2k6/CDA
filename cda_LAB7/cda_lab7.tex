...
\begin{itemize}
  \item Two 74LS74 dual positive edge-triggered DFF ICs (provides $Q$ and $\overline{Q}$
        outputs directly; asynchronous CLR used for reset)
  \item One 74LS00 quad 2-input NAND gate IC
  \item Three LEDs and current-limiting resistors (for original circuit)
  \item 74LS47N BCD-to-7-segment decoder/driver and common-anode 7-segment display
        (for modified circuit)
  \item One push button and pull-down resistor (for reset)
...
Since DFF characteristic equation is $Q^* = D$, the D inputs equal the desired next
states directly.  We filled in three Karnaugh maps to obtain minimized SOP expressions
for $D_2$, $D_1$, and $D_0$, then converted them to NAND-NAND form for implementation.
The $\overline{Q}$ outputs of the 74LS74 supplied all required complemented state signals,
eliminating the need for a separate inverter IC.  After confirming the design with the
TA, we wired the circuit on the breadboard, connected a 1\,Hz clock from the function
generator, and verified correct operation using the LED outputs.  In the extension, we
replaced the LEDs with the 74LS47N and 7-segment display.
...
\subsubsection*{NAND-NAND Implementation}

Converting all three equations to NAND-NAND two-level form (using De Morgan's theorem,
$A+B = \overline{\bar{A}\cdot\bar{B}}$):

\begin{align*}
  D_2 &= \overline{\overline{Q_1\overline{Q_0}} \cdot \overline{Q_2 Q_0}} \\
  D_1 &= \overline{\overline{\overline{Q_2}\,Q_0} \cdot \overline{Q_1\overline{Q_0}}} \\
  D_0 &= \overline{\overline{\overline{Q_2}\,\overline{Q_1}} \cdot \overline{Q_2 Q_1}}
\end{align*}

All complemented signals ($\overline{Q_2}$, $\overline{Q_1}$, $\overline{Q_0}$) are
taken directly from the $\overline{Q}$ outputs of the 74LS74, requiring no extra
inverter.  The term $\overline{Q_1\overline{Q_0}}$ appears in both $D_2$ and $D_1$ and
is computed once and shared.  In total, 5 two-input NAND gates are needed: two 74LS00
ICs suffice (8 gates available, 5 used, one gate shared between $D_2$ and $D_1$).
...
The original circuit uses two 74LS74 ICs (three of the four available DFFs are used for
$Q_2$, $Q_1$, $Q_0$) and one 74LS00 quad-NAND IC.  The key wiring connections are:
...
\begin{tabular}{ll}
  ...
  \midrule
  CLK (all 3 DFFs) & Function generator 1\,Hz CMOS output \\
  $\overline{\text{CLR}}$ (all 3 DFFs) & Reset button (active-low; pull-up to $V_{DD}$) \\
  $\overline{\text{PRE}}$ (all 3 DFFs) & $V_{DD}$ (disabled) \\
  $D_2$ & NAND output: $\overline{\overline{Q_1\overline{Q_0}}\cdot\overline{Q_2Q_0}}$ \\
  $D_1$ & NAND output: $\overline{\overline{\overline{Q_2}Q_0}\cdot\overline{Q_1\overline{Q_0}}}$ \\
  $D_0$ & NAND output: $\overline{\overline{\overline{Q_2}\,\overline{Q_1}}\cdot\overline{Q_2Q_1}}$ \\
...
The K-map simplification produced compact excitation equations.  The shared term
$Q_1\overline{Q_0}$ between $D_2$ and $D_1$ reduced the gate count by one, allowing the
entire combinational logic to fit within a single 74LS00 IC (5 of 8 available gates
used).  Since the 74LS74 provides $\overline{Q}$ outputs natively, no separate inverter
IC was required, keeping the design to three ICs total.

The expression $D_0 = \overline{Q_2}\,\overline{Q_1} + Q_2 Q_1 = \overline{Q_2 \oplus Q_1}$
has an intuitive interpretation: the LSB of the next Gray code state is 1 whenever the
two MSBs of the current state are equal, and 0 otherwise.  This reflects the structure
of the Gray code where the lower bits ``mirror'' as the count progresses through the
...
In this lab we designed and implemented a 3-bit Gray code up counter using D flip-flops.
We derived the next-state table from the Gray code sequence, simplified the excitation
equations for $D_2$, $D_1$, and $D_0$ using Karnaugh maps, and converted them to
NAND-NAND two-level logic.  The final hardware used only three ICs: two 74LS74 DFF ICs
(three flip-flops, providing $\overline{Q}$ outputs that eliminated the need for a
separate inverter) and one 74LS00 NAND IC (with one gate shared between $D_2$ and
$D_1$).  A Verilog behavioral model and testbench confirmed the expected waveform.  The
breadboard circuit, clocked at 1\,Hz, successfully cycled through all eight Gray code
states with exactly one LED changing per clock edge.  The optional extension connected
the counter to a 74LS47N and 7-segment display, further demonstrating the circuit's
correct operation.
