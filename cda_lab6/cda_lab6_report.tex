...
\begin{center}
  {\Large \textbf{Report for CDA 3201L Lab \#6}}\\[10pt]
  {\large \textbf{Sequential Circuits -- Game Show Circuit}}\\[20pt]
  \begin{tabular}{ll}
...
\begin{itemize}
  \item 74LS74 dual positive edge-triggered D flip-flop IC (provides both $Q$ and $\overline{Q}$ outputs, eliminating the need for a separate inverter)
  \item 74LS08 quad 2-input AND gate IC
  \item 74LS32 quad 2-input OR gate IC
  \item Breadboard and jumper wires
  \item Two LEDs and resistors
  \item Three push buttons (contestant inputs $X_0$ and $X_1$, plus a reset button)
  \item 5\,V DC power supply
  \item Function generator (for the clock signal)
  \item Oscilloscope
\end{itemize}

First, we derived the state transition table and excitation equations from the lab specification. We then filled in the Karnaugh maps for $Q_0^*$ and $Q_1^*$ and simplified the expressions. Using the 74LS74 datasheet, we identified the pin-out for the DFFs, including the asynchronous clear (CLR) pin used for reset. Importantly, the 74LS74 provides both $Q$ and $\overline{Q}$ outputs, so the complemented state bits ($\overline{Q_0}$ and $\overline{Q_1}$) needed in the excitation equations are available directly from the flip-flop without requiring a separate inverter IC. We configured the function generator to output a square wave at an appropriate frequency and amplitude to serve as the clock. The circuit was wired on the breadboard using only three ICs (74LS74, 74LS08, 74LS32), connecting the combinational logic for the D inputs of each flip-flop, the clock from the function generator, and a push button for reset via the CLR pins. The Q outputs of the flip-flops were connected directly to LEDs through current-limiting resistors.

\section{Results}

\subsection{State Transition Diagram and K-map for $Q_1^*$}

The game show circuit has four states: State A (wait, $Y_1Y_0 = 00$), State B (contestant 0 wins, $Y_1Y_0 = 01$), State C (contestant 1 wins, $Y_1Y_0 = 10$), and State D (tie, $Y_1Y_0 = 11$). The state transition diagram is shown below:

\begin{figure}[H]
\centering
\begin{tikzpicture}[->, >=Stealth, node distance=3.5cm, thick,
    every state/.style={minimum size=1.2cm, font=\small}]

  \node[state, initial, initial text=reset, initial where=left] (A) {$\begin{array}{c} A \\ 00 \end{array}$};
  \node[state, below right of=A] (B) {$\begin{array}{c} B \\ 01 \end{array}$};
  \node[state, above right of=A] (C) {$\begin{array}{c} C \\ 10 \end{array}$};
  \node[state, right of=A, node distance=5.5cm] (D) {$\begin{array}{c} D \\ 11 \end{array}$};

  % From A
  \path (A) edge [loop above] node[above] {00} (A)
        (A) edge [bend right] node[below left] {01} (B)
        (A) edge [bend left] node[above left] {10} (C)
        (A) edge node[above] {11} (D);

  % Self-loops for B, C, D (all inputs)
  \path (B) edge [loop below] node[below] {XX} (B);
  \path (C) edge [loop above] node[above] {XX} (C);
  \path (D) edge [loop right] node[right] {XX} (D);

\end{tikzpicture}
\caption{State transition diagram for the game show circuit. Each state is labeled with its name and encoding ($Q_1Q_0$). Transitions are labeled with the input $X_1X_0$. States B, C, and D have self-loops on all inputs (denoted XX), meaning once entered, the state is locked until reset.}
\end{figure}

The state transition table formalizing this behavior is:
...
\begin{itemize}
  \item \textbf{Group 1 (8 cells):} All eight cells in rows $Q_1Q_0 = 10$ and $Q_1Q_0 = 11$ are 1. These two rows share $Q_1 = 1$, so this group simplifies to the single term $Q_1$.
  \item \textbf{Group 2 (4 cells):} The two cells at ($Q_1Q_0 = 00$, $X_1X_0 = 10$) and ($Q_1Q_0 = 00$, $X_1X_0 = 11$) combine with the adjacent cells at ($Q_1Q_0 = 10$, $X_1X_0 = 10$) and ($Q_1Q_0 = 10$, $X_1X_0 = 11$). These four cells share $Q_0 = 0$ and $X_1 = 1$, giving the term $X_1 \cdot \overline{Q_0}$.
\end{itemize}
...
To return to the wait state, a third push button is used to assert the asynchronous CLR input on the 74LS74 (active low), which forces both flip-flop outputs to 0 regardless of the clock or D inputs.

\subsection{Physical Circuit Testing}

The circuit was built on a breadboard using only three ICs: the 74LS74 DFF IC, a 74LS08 AND gate IC, and a 74LS32 OR gate IC. No inverter IC was needed because the $\overline{Q}$ outputs of the 74LS74 directly provide the complemented state bits. The function generator was connected to the clock input of both flip-flops, and an oscilloscope probe was used to verify the clock signal.
...
\begin{center}
  \small\textbf{Supplementary Video:} \href{https://drive.google.com/file/d/1QkF50zO321dzvna1qJGpwtyxzRFBLg6M/view}{Circuit Demonstration Recording (Google Drive)}
\end{center}
...
In this lab, we designed and implemented a sequential game show circuit using D flip-flops. The circuit correctly determines which contestant rings in first and locks the result until reset. We derived the excitation equations using Karnaugh maps and implemented the combinational logic with only three ICs: a 74LS74 DFF (whose $\overline{Q}$ outputs provided the needed complements, eliminating the need for a separate inverter), a 74LS08 AND gate, and a 74LS32 OR gate. Three push buttons served as inputs for contestant 0, contestant 1, and reset. A function generator provided the clock signal. The lab reinforced our understanding of sequential circuit design, including state transition tables, excitation equations, and the practical considerations of clocking and resetting flip-flop based circuits.
